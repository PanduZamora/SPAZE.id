...
class NEXT_3D_FlipBox extends ET_Builder_Module {
	...
	public function init() {
		$this->name 		= esc_html__( 'Next 3D Flip Box', 'dnxte-divi-essential' );
		...
		$this->settings_modal_toggles = array(
			'general'	=> array(
				'toggles'		=>	array(
					'dnext3d_flipbox'	=> array(
						'title'		=>	esc_html__( '3d Flip Elements', 'dnxte-divi-essential' ),
						'priority'	=>	1,
						'sub_toggles'       => array(
					...
					'bgc_front'	=> array(
						'title'		=>	esc_html__( '3D Flipbox Front Background', 'dnxte-divi-essential' ),
						'priority'	=>	70,
						'sub_toggles'       => array(
					...
					'bgc_back'	=> array(
						'title'		=>	esc_html__( '3D Flipbox Back Background', 'dnxte-divi-essential' ),
						'priority'	=>	70,
						'sub_toggles'       => array(
                            'sub_toggle_color'   => array(
								'icon' => 'background-color',
			...
			'advanced'	=> array(
				'toggles'	=>	array(
					...
					'front_border'	=> array(
						'title'		=>	esc_html__( 'Front Border', 'dnxte-divi-essential' ),
						'priority'	=>	91,
					),
					'back_border'	=> array(
						'title'		=>	esc_html__( 'Back Border', 'dnxte-divi-essential' ),
						'priority'	=>	92,
					),
		...
		$this->advanced_fields = array(
			...
			'borders'               => array(
				'default' => array(
					'css'          => array(
						'main'     => array(
							'border_radii'  		=> "%%order_class%% .dnext-3d-transform-flpb-box-item",
							'border_radii_hover'  	=> '%%order_class%%:hover .dnext-3d-transform-flpb-box-item',
							'border_styles' 		=> "%%order_class%% .dnext-3d-transform-flpb-box-item",
							'border_styles_hover' 	=> '%%order_class%%:hover .dnext-3d-transform-flpb-box-item',
						),
					),
					'defaults'        => array(
			...
			'box_shadow'            => array(
				'default' => array(
					'css'                 => array(
						'main'        => "%%order_class%% .dnext-3d-transform-flpb-flip-box",
						'hover'       => '%%order_class%%:hover .dnext-3d-transform-flpb-flip-box',
						'overlay'     => 'inset',
					),
				),
	...
	public function get_fields() {
		$frontend_fields = array(
			...
			'front_image_color' 	=> array(
				'label'             => esc_html__( 'Image Backgrond Color', 'dnxte-divi-essential' ),
				'type'              => 'color-alpha',
				'default'			=> '',
		...
		$backend_fields = array(
			...
			'flipbox_effect'	=> array(
				...
				'option_category'   => 'layout',
				'options'           => array(
					''										=> esc_html__( 'Right Left', 'dnxte-divi-essential' ),
					'dnext-3d-transform-flpb-rightto-left'	=> esc_html__( 'Left Right', 'dnxte-divi-essential' ),
					'dnext-3d-transform-flpb-bottomto-top'	=> esc_html__( 'Bottom Top', 'dnxte-divi-essential' ),
				),
				'default'            => '',
			),
			// Content Background Gradient 
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		if ( 'off' !== $this->props['front_bgi_show_hide'] && "off" !== $this->props['front_overlay_show_hide']) {
			...
			$front_bg_image_style 		 	= sprintf( 'background: %1$s, url("%2$s");', $front_overlay_style, esc_url( $front_bg_image ) );
		...
		if ( 'off' !== $front_icon_switch ) {
			...
			$fronticon = '<span class="et-waypoint et_pb_animation_top et-animated '.esc_attr( $front_icon_alignment_classes ).'" data-icon="'.esc_attr( et_pb_process_font_icon( $front_icon )).'"></span>';
			
			ET_Builder_Element::set_style($render_slug, array(
				'selector'    	=> "%%order_class%% .dnext-3d-flpb-flibbox-icon-front span::before",
				'declaration'	=> 'content: attr(data-icon);font-family: ETmodules!important;'
			) );
		}
		if( 'off' !== $front_image_switch) {
			$fronticon .= sprintf('<div class="dnext-3d-flipbox-image %3$s"><img src="%1$s" alt="%2$s" class="dnext-3d-flipbox-front-image"/></div>', esc_attr__($front_image, 'dnxte-divi-essential'), esc_attr__($front_image_alt, 'dnxte-divi-essential'), esc_attr__($front_image_alignment_classes, 'dnxte-divi-essential'));
		}
		...
		if ( "off" !== $this->props['btn_show_hide'] ) {
			...
			ET_Builder_Element::set_style( $render_slug, array(
				...
				'declaration' => "background-color: {$this->props['btn_bg_color']};",
			) );
			...
			if ( "" !== $btn_bg_color_style_hover ) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => $this->add_hover_to_order_class( $btn_selector ),
		...
		if ( 'off' !== $back_icon_switch ) {
			...
			$backicon = '<span class="et-waypoint et_pb_animation_top et-animated '.esc_attr( $back_icon_alignment_classes ).'" data-icon="'.esc_attr( et_pb_process_font_icon( $back_icon )).'"></span>';
		...
		if( 'off' !== $back_image_switch) {
			$backicon .= sprintf('<div class="dnext-3d-flipbox-image %3$s"><img src="%1$s" alt="%2s" class="dnext-3d-flipbox-back-image"/></div>', esc_attr__($back_image, 'dnxte-divi-essential'), esc_attr__($back_image_alt, 'dnxte-divi-essential'), esc_attr__($back_image_alignment_classes, 'dnxte-divi-essential'));
		}
