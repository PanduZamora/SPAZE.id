...
class Next_Blurb extends ET_Builder_Module {
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		if ( 'off' !== $dnxt_use_icon ) {
			...
			$fonticon = sprintf('<div class="dnxt-blurb-icon %1$s"><span class="et-waypoint et_pb_animation_top et-pb-icon et-animated" data-icon="'.esc_attr( et_pb_process_font_icon( $this->props["dnxt_font_icon"] )).'"></span></div>',
			$fontclass
		);
		} else {
			$fonticon = "";
		...
		if ( 'off' !== $this->props['dnxt_btn_show_hide'] ) {
			...
			if ( 'right' === $this->props['btn_icon_placement'] ) {
				$rightItag = '<i data-icon='.$dataIcon.'></i>';
			}else if ( 'left' === $this->props['btn_icon_placement'] ) {
				$leftItag = '<i data-icon='.$dataIcon.'></i>';
			}
	...
	public function apply_css($render_slug){
        ...
        */
		...
		if ( 'off' !== $this->props['heading_bg_show_hide'] ) {
			$heading_bg_color_style 		= sprintf( 'background-color: %1$s;', esc_attr__( $heading_bg_color, 'dnxte-divi-essential' ) );
			$heading_bg_color_tablet_style 	= '' !== $heading_bg_color_tablet ? sprintf( 'background-color: %1$s;', esc_attr__( $heading_bg_color_tablet, 'dnxte-divi-essential' ) ) : '';
			$heading_bg_color_phone_style  	= '' !== $heading_bg_color_phone ? sprintf( 'background-color: %1$s;', esc_attr__( $heading_bg_color_phone, 'dnxte-divi-essential' ) ) : '';
			
			$heading_bg_color_style_hover  = '';

			if ( et_builder_is_hover_enabled( 'heading_bg', $this->props ) ) {
				$heading_bg_color_style_hover = sprintf( 'background-color: %1$s;', esc_attr__( $heading_bg_color_hover, 'dnxte-divi-essential' ) );
			}
	...
	if ( 'off' !== $this->props['use_image_bg_color'] ) {
		...
		if( "on" === $this->props['btn_show_hide'] ) {
			...
			if ( 'right' === $this->props['btn_icon_placement'] ) {
				ET_Builder_Element::set_style($render_slug, array(
					'selector'    	=> "%%order_class%% .dnxt-btn-icon i::before",
			...
			if ( "on|hover" === $btn_icon_color_hover ) {
				ET_Builder_Element::set_style($render_slug, array(
					'selector'    => "%%order_class%% .dnxt-btn-icon:hover i:before",
			...
			if ( 'off' === $this->props['btn_on_hover'] && 'left' === $this->props['btn_icon_placement'] ) {
				ET_Builder_Element::set_style($render_slug, array(
					'selector'    => "%%order_class%% .dnxt-btn-icon.dnxt-btn-icon-on-hover i:before",
