...
class DNEXT_Divider extends ET_Builder_Module {
	...
	public function init() {
		$this->name 		= esc_html__( 'Next Divider', 'dnxte-divi-essential' );
		$this->icon_path	= plugin_dir_path( __FILE__ ) . 'icon.svg';

		$this->settings_modal_toggles = array(
			'general'  => array(
				'toggles' => array(
					'dnext_sid_image'       	=> esc_html__( 'Image & Icon', 'dnxte-divi-essential' ),
					'dnext_divider_background'	=> array(
						'title'		=>	esc_html__( 'Background Color', 'dnxte-divi-essential' ),
						'priority'	=>	78,
						'sub_toggles'       => array(
			...
			'advanced' => array(
				'toggles' => array(
					'dnext_sid_icon_settings' 	=> esc_html__( 'Image & Icon', 'dnxte-divi-essential' ),
					'dnext_sid_divider_style'	=> esc_html__( 'Divider Style', 'dnxte-divi-essential' ),
				),
			),
			'custom_css' => array(
				'toggles' 						=> array(),
	...
	public function get_fields() {
		...
		return array(
			...
			'dnext_sid_icon_alignment' => array(
				'label'           => esc_html__( 'Image/Icon Alignment', 'dnxte-divi-essential' ),
			...
			'dnext_sid_divider_width' 	=> array(
				'label'             => esc_html__( 'Divider Width', 'dnxte-divi-essential' ),
				'type'              => 'range',
				'option_category'   => 'layout',
				'depends_show_if'   => 'on',
				'tab_slug'          => 'advanced',
			...
			'dnext_sid_divider_gap' => array(
				'label'             => esc_html__( 'Divider Gap', 'dnxte-divi-essential' ),
				'type'              => 'range',
				'option_category'   => 'layout',
				'tab_slug'          => 'advanced',
				'toggle_slug'       => 'dnext_sid_divider_style',
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$dnext_sid_font_icon 				  = esc_attr( et_pb_process_font_icon($this->props['dnext_sid_font_icon']));
		$dnext_sid_icon_font_icon_hover 	  = esc_attr( et_pb_process_font_icon($this->get_hover_value( 'dnext_sid_font_icon' )));
		$dnext_sid_icon_font_icon_values	  = et_pb_responsive_options()->get_property_values( $this->props, 'dnext_sid_font_icon' );
		$dnext_sid_icon_font_tablet		  	  = isset( $dnext_sid_icon_font_icon_values['tablet'] ) ? $dnext_sid_icon_font_icon_values['tablet'] : '';
		$dnext_sid_icon_font_phone			  = isset( $dnext_sid_icon_font_icon_values['phone'] ) ? $dnext_sid_icon_font_icon_values['phone'] : '';
		...
		if ( 'off' !== $dnext_sid_use_icon ) {
			...
			$image_icon 		 	 = sprintf( '<div class="%2$s">%1$s</div>', esc_attr( $dnext_sid_font_icon ), $dnext_sid_icon_class );
			$image_icon_tablet_style = '' !== $dnext_sid_icon_font_tablet ? sprintf( '<div class="%2$s">%1$s</div>', esc_attr( $this->props['dnext_sid_font_icon_tablet'] ),$dnext_sid_icon_class ) : '';
			$image_icon_phone_style  = '' !== $dnext_sid_icon_font_phone ? sprintf( '<div class="%2$s">%1$s</div>', esc_attr( $dnext_sid_icon_font_phone ),$dnext_sid_icon_class ) : '';
			
			
			if ( et_builder_is_hover_enabled( 'dnext_sid_font_icon', $this->props ) ) {
				$dnext_sid_icon_hover = sprintf( '<div class="dnext-sid-text-divider-icon-hover">%1$s</div>', esc_attr( $dnext_sid_icon_font_icon_hover ) );
			}
