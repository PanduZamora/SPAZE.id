...
jQuery(document).ready(function($) {
  ...
  (function($) {
    ...
    $(".dnxte-twitter-tweet").each(function(e){
      ...
      window.twttr && window.twttr.widgets.createTweet(tweetData.id, this, {
        theme: tweetData.themeName,
        cards: tweetData.showCards === "on" ? "" : "hidden",
        conversation:
          tweetData.showConversation === "on" ? "" : "none",
    ...
    // Promobox shapes
    getShapeFrontend($,"dnxte-promobx-thumb");
    // Stepflow shapes
    getShapeFrontend($,"dnxte-stepflow-image-wrap");
    // Blurb shapes
    getShapeFrontend($,"dnxt-blurb-image");
    // Flipbox shapes
    getShapeFrontend($,"dnext-flipbox-front-image");
    getShapeFrontend($,"dnext-flipbox-back-image");
